...
if(isset($_GET['validatenewUser']))
{
	...
	if($isUsernameValid && $isPwdValid && $isemailValid){
		header('Location: listingPage.html.php?username='.urlencode($username).'&password='.urlencode($password).'&email='.urlencode($email).'&zip='.urlencode($zip).'&telephone='.urlencode($telephone));
	}
	else
	{
		?>
